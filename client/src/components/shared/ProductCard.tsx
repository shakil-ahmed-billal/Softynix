...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const gradientColor = gradientColors[Number(product.id) % gradientColors.length];
  ...
  const toggleFavorite = (e: React.MouseEvent) => {
    ...
    toast.success(isFavorite ? "ফেভারিট থেকে সরানো হয়েছে" : "ফেভারিটে যোগ করা হয়েছে");
  };
  ...
  return (
    ...
    >
      ...
      <div className="relative p-3 md:p-4 pb-0">
        ...
        <div className={`w-full aspect-square rounded-xl md:rounded-2xl bg-gradient-to-br ${gradientColor} flex items-center justify-center shadow-lg group-hover:shadow-2xl transition-all duration-500 overflow-hidden relative`}>
          {/* Decorative circles */}
      ...
      <CardContent className="p-3 md:p-4 space-y-2 md:space-y-2.5">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs md:text-sm font-semibold text-foreground">{rating}</span>
