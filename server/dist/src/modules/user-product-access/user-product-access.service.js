...
export class UserProductAccessService {
     ...
     */
    async getUserPurchases(userId, pagination = {}, filters = {}) {
     ...
     */
    async getUserAISubscriptions(userId) {
     ...
     */
    async getUserSoftwareLicenses(userId) {
        const accesses = await prisma.userProductAccess.findMany({
            where: {
                userId,
     ...
     */
    async getUserProductivityApps(userId) {
        const accesses = await prisma.userProductAccess.findMany({
            where: {
                userId,
     ...
     */
    async getProductAccessById(accessId, userId) {
     ...
     */
    async updateCourseProgress(accessId, userId, progress) {
     ...
     */
    async getLessonCompletions(accessId, userId) {
        // Verify ownership
        const access = await prisma.userProductAccess.findFirst({
            where: {
                id: accessId,
     ...
     * Complete a lesson
     */
    async completeLesson(accessId, userId, milestoneId, moduleId) {
        // Verify ownership
        const access = await prisma.userProductAccess.findFirst({
            where: {
                id: accessId,
        ...
        const totalLessons = courseData.milestones?.reduce((sum, milestone) => sum + (milestone.modules?.length || 0), 0) || 1;
     ...
     */
    async markLessonViewed(accessId, userId, milestoneId, moduleId) {
        // Verify ownership
        const access = await prisma.userProductAccess.findFirst({
            where: {
                id: accessId,
...
//# sourceMappingURL=user-product-access.service.js.map